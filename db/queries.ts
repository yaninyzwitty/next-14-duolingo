...
export const getCourseProgress = cache(async () => {
  ...
  return {
    aciveLesson: firstUncompletedLesson,
...
export const getLesson = cache(async  (id?: number) => {
  ...
  const normalizedChallenges = data.challenges.map((challenge) => {
    const completed = challenge.challengeProgress && challenge.challengeProgress.length > 0 && challenge.challengeProgress.every((progress) => progress);
